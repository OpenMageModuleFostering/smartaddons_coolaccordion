...
class Smartaddons_Coolaccordion_Block_List extends Mage_Core_Block_Template
{
	...
	public function __construct($attributes = array()){
		parent::__construct();
		$this->_config = Mage::helper('coolaccordion/data')->get($attributes);
	}

	public function getConfig($name=null, $value=null){
		if (is_null($this->_config)){
			$this->_config = Mage::helper('coolaccordion/data')->get(null);
		}
	...
	protected function _prepareLayout(){
		...
		//$template_file = "smartaddons/coolaccordion/block_template_default.phtml";
		$template_file = "smartaddons/coolaccordion/default.phtml";
	...
	protected function _getMedia(){
		$items = Mage::getStoreConfig('coolaccordion_cfg/product_selection/product_additem');	//kq:  string(85) "a:1:{s:18:"_1289468104191_191";a:2:{s:6:"regexp";s:4:"6:20";s:5:"value";s:4:"8:20";}}" 
		if($items){
			...
			foreach($array_items as $key=>$item){
				...
				if($this->_config['item_image_disp']){
					$item['thumb_small'] = (string)$this->_getResizedImage($item['image'],$this->getConfig('item_image_width'), $this->getConfig('item_image_height'),100);
				}else{
					$item['thumb_small'] ='';
				}			
		...
		foreach( $products as $k => $_product ) {
			...
			if($this->_config['product_image_disp']){
				if($_product->getImage()){
					...
					$item['thumb'] = $this->getSkinUrl('smartaddons/coolaccordion/images/no_image.png');
				}	
			}else{
				$item['thumb'] ='';
			}
			if($this->_config['item_image_disp']){
				if($_product->getImage()){
					$item['thumb_small'] = (string)Mage::helper('catalog/image')->init($_product, 'image')->resize($this->getConfig('item_image_width'), $this->getConfig('item_image_height'));
				}
				else{
					$item['thumb_small'] = $this->getSkinUrl('smartaddons/coolaccordion/images/no_image.png');
				}	
			}else{
				$item['thumb_small'] ='';
			}			
            $item['content'] = $_product->getShortDescription();
 			$item['sub_content'] = Mage::helper('core/string')->truncate( $item['content'] , $this->_config['product_description_max_characters'],'...');
			$item['price'] = Mage::helper('core')->currency($_product->getPrice(), true, false);
			$collect_items[] = $item;
		}	
		return $collect_items;
	}
	...
	public function getCssHead(){
		$assets_items = array();
		$assets_path	= "smartaddons/coolaccordion/";	
		$theme	= $this->getConfig('theme');
		//add setting size and default values for module
		if ($theme=='theme1'){
				$useragent = $_SERVER['HTTP_USER_AGENT'];
				preg_match('|MSIE ([0-9].[0-9]{1,2})|',$useragent,$matched);
				if (isset($matched[1]) && $matched[1]=='7.0'){
					if (null == Mage::registry('smartaddons.coolaccordion.theme1_IE7_css')){
						$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme1_IE7.css');
						Mage::register('smartaddons.coolaccordion.theme1_IE7_css', 'included');
					}
				}
				else{
					if (null == Mage::registry('smartaddons.coolaccordion.theme1_css')){
						$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme1.css');
						Mage::register('smartaddons.coolaccordion.theme1_css', 'included');
					}
				}
		}
		if ($theme=='theme2'){
			if (null == Mage::registry('smartaddons.coolaccordion.theme2_css')){
				$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme2.css');
				Mage::register('smartaddons.coolaccordion.theme2_css', 'included');
			}
		}
		if ($theme=='theme3'){
			if (null == Mage::registry('smartaddons.coolaccordion.theme3_css')){
				$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme3.css');
				Mage::register('smartaddons.coolaccordion.theme3_css', 'included');
			}
		}
		if ($theme=='theme4'){
				$useragent = $_SERVER['HTTP_USER_AGENT'];
				preg_match('|MSIE ([0-9].[0-9]{1,2})|',$useragent,$matched);
				if (isset($matched[1]) && $matched[1]=='9.0'){
					if (null == Mage::registry('smartaddons.coolaccordion.theme4_IE9_css')){
						$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme4_IE9.css');
						Mage::register('smartaddons.coolaccordion.theme4_IE9_css', 'included');
					}
				}
				else{
					if (null == Mage::registry('smartaddons.coolaccordion.theme4_css')){
						$assets_items[] = array('type' => 'skin_css', 'name' => $assets_path.'css/theme4.css');
						Mage::register('smartaddons.coolaccordion.theme4_css', 'included');
					}
				}
		}		
		// echo $script;
		...
		foreach ($assets_items as $item){
			//$headBlock->addItem($item['type'], $item['name']);
			$import_str.='<link rel="stylesheet" type="text/css" href="'.$this->getSkinUrl($item['name']).'" media="all" />';
		}	
		return $import_str;
	}
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('coolaccordion_cfg/advanced/include_jquery')){
				// if module allowed jquery.
				$import_str .= $jsHelper->includeSkinScript('smartaddons/coolaccordion/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('smartaddons/coolaccordion/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.coolaccordion.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('smartaddons/coolaccordion/js/ytc.accordion.theme2_3.js');
			$import_str .= $jsHelper->includeSkinScript('smartaddons/coolaccordion/js/ytc.hrzAccordion.js');
			Mage::register('jsmart.coolaccordion.js', 1);
		}
		return $import_str;
	}
	...
	private function _getResizedImage($Obj, $width, $height, $quality = 100) {
        ...
        if(file_exists($imageResized)){
            ...
            return $this->getSkinUrl('smartaddons/coolaccordion/images/no_image.gif');
        }
    }	
}
